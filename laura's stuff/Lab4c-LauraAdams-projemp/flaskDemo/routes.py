...
from flaskDemo.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, DeptForm,DeptUpdateForm, ProjForm, ProjUpdateForm
...
def home():
    ...
    results2 = Works_On.query.all()
    return render_template('assign_home.html', joined_m_n = results, idk = results2)
...
@app.route("/create_assign", methods=['GET','POST'])
@login_required
def create_assign():
    form = ProjForm()
    proj = Works_On(essn=form.essn.data, pno=form.pno.data, hours = form.hours.data)
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.commit()
            flash('You have created a new assignment!', 'success')
            return redirect(url_for('home'))
        except IntegrityError:
            db.session.rollback()
            flash('Project assignment already exists!', 'danger')

    return render_template('create_assign.html', title='Create Assignment', form=form, legend='Create Assignment')
...
def delete_assign(essn,pno):
    ...
    db.session.commit()
    flash('Employee has been removed from that project!', 'success')
    return redirect(url_for('home'))
    
@app.route("/assign/<essn>/<pno>/update", methods=['GET','POST'])
@login_required
def update_assign(essn,pno):
    flash('Update page under construction.', 'danger')
    return redirect(url_for('home'))